...
function normalizeLink(url) {
  ...
  if (url.includes("dropbox.com")) {
    return url.replace("www.dropbox.com", "dl.dropboxusercontent.com").replace("?dl=0", "").trim();
  }
  return url;
}
...
function loadTrack(i){
  ...
  audio.src = normalizeLink(track.audio);
  coverEl.style.backgroundImage=`url('${normalizeLink(track.cover)}')`;
...
function renderPlaylist(){
  ...
  demoPlaylist.forEach((t,idx)=>{
    ...
    const thumbWrap=document.createElement('div');thumbWrap.className='gold-ring rounded-lg p-0.5';
    const thumb=document.createElement('div');thumb.className='w-10 h-10 rounded-md overflow-hidden bg-black/60';
    thumb.style.backgroundImage=`url('${t.cover}')`;thumb.style.backgroundSize='cover';thumb.style.backgroundPosition='center';
    thumbWrap.appendChild(thumb);
    const meta=document.createElement('div');meta.className='min-w-0';
    const tt=document.createElement('div');tt.className='text-amber-50 font-semibold truncate';tt.textContent=t.title;
    const ar=document.createElement('div');ar.className='text-amber-200/80 text-sm truncate';ar.textContent=t.artist;
    meta.appendChild(tt);meta.appendChild(ar);
    li.appendChild(thumbWrap);li.appendChild(meta);playlistEl.appendChild(li);
  });
}
